...
@for($bulan = 1; $bulan <= 12; $bulan++)
    <h3>{{ \Carbon\Carbon::create()->month($bulan)->locale('id')->translatedFormat('F') }}</h3>
    ...
    @if(isset($topPerBulan[$bulan]))
        <p>
            Booking terbanyak bulan ini:
            @foreach ($topPerBulan[$bulan] as $top)
                {{ $top->kategori_paket }} ({{ $top->jumlah_booking }} booking)
                @if(!$loop->last), @endif
            @endforeach
        </p>
    @endif
...
@if(isset($topTahunan))
    <div class="summary">
        <p>
        Booking terbanyak tahun ini:
        {{ $topTahunan->kategori_paket }}
