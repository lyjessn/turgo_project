...
<table>
    ...
    @if(isset($grouped[$bulan]))
        ...
        <tr>
            <td>{{ ucfirst(str_replace('_',' ',$row->tipe_booking)) }}</td>
...
<p>
    Tipe booking terbanyak bulan ini:

    @foreach($topPerBulan[$bulan] as $top)
        {{ ucfirst(str_replace('_',' ',$top->tipe_booking)) }}
        ({{ $top->jumlah_booking }} booking)
        @if(!$loop->last), @endif
    @endforeach
...
<div class="summary">
    <p>
        Tipe booking terbanyak tahun ini:
        {{ ucfirst(str_replace('_',' ',$topTahunan->tipe_booking)) }}
